...
import { LfgChannelTable, LfgGameTable, LfgPostFullRow } from "../../Interfaces/lfg_system.js";
import { embed_error, embed_message } from "../../utility_modules/embed_builders.js";
import { errorLogHandle } from "../../utility_modules/error_logger.js";
import { bump_lfg_post, delete_lfg_post, fetchPostMessage, lfg_post_builder, lfg_post_buttons, stringifyRoles } from "./lfg_post.js";
...
export async function active_lfg_posts(
    ...
    const postsOnChannel = posts.filter(p => p.discord_channel_id === channel.id);
    if (!postsOnChannel.length) {
    ...
    for (const row of postsOnChannel) {
        ...
        try {
            ...
            activePostStrings.push(
                `${index++} - ${owner.toString()} \`+${row.slots}\` ${row.gamemode_name} ${rankString} [jump to post](${lfg.message.url})`
            );
 ...
 */
export async function interface_manager_collector(message: Message, lfgChannelTable: LfgChannelTable) {
    ...
    const collector = await message_collector<ComponentType.Button>(message,
        ...
        async (buttonInteraction) => {
            ...
            // fetch the game row
            const gameTable: LfgGameTable | null = await LfgSystemRepo.getGameByInterface(message.id);
            if (!gameTable) {
                await buttonInteraction.reply({
                    embeds: [
                        embed_error(
            ...
            switch (buttonInteraction.customId) {
                ...
                case "active-lfg-posts": {
                    ...
                    const activeLFGs = await active_lfg_posts(
                        guild,
                        gameTable,
                        (buttonInteraction.member as GuildMember),
                        buttonInteraction.channel as TextChannel
                    );
...
export function interface_embed_components(gameName: string): MessageCreateOptions {
    return {
        embeds: [embed_interface_manager(gameName)],
        components: [
...
export async function send_lfg_interface(channel: TextChannel, gameName: string, lfgChannelTable: LfgChannelTable) {
    const lfgInterface = await channel.send(interface_embed_components(gameName));
    // activate interface collector
    await interface_manager_collector(lfgInterface, lfgChannelTable);
    // register interface
    const lfgInterfaceTable = await LfgSystemRepo.registerInterface(lfgInterface.id, lfgChannelTable.id);
    return { row: lfgInterfaceTable, message: lfgInterface };
}

export async function handle_delete_interface(message: Message) {
    await LfgSystemRepo.deleteInterface(message.id);
    try {
