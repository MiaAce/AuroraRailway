...
import { send_lfg_interface } from "./lfg_interface_manager.js";
...
export async function register_and_build_lfg(
    ...
    for (const name of channels) {
        ...
        const registeredChannel = await LfgSystemRepo.registerChannel(lfgChannel);
        // send the interface
        await send_lfg_interface(channel, game.game_name, registeredChannel);
    }
        ...
        async (buttonInteraction) => {
            ...
            switch (buttonInteraction.customId) {
                ...
                case "builder-button": {
                    ...
                    if (checkGame && checkGame.category_channel_id !== null) { // safe guarding against building the same game more than once
                        await buttonInteraction.reply({
                            embeds: [embed_error(
                                "It seems like another builder instance already built this game before the button was pressed.\n" +
                                `Channel Id: ${checkGame.category_channel_id}`
                            )]
                        });
                        collector.stop();
                        return;
