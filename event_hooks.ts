...
import { handle_delete_interface, send_lfg_interface } from "./lfg_interface_manager.js";
...
export async function bump_lfg_post(message: Message, post: LfgPostTable): Promise<number> {
    if (message.channel.isSendable()) { // not needed, but it makes typescript happy
        ...
        // TODO: ADD PROPER DATA HANDLING
        // delete and re-send the interface
        const interfaceTable = await LfgSystemRepo.getInterfaceByChannelId(post.channel_id);
        if (interfaceTable) {
            const game = await LfgSystemRepo.getGameByInterface(interfaceTable.message_id);
            const lfgChannelTable = await LfgSystemRepo.getLfgChannelById(interfaceTable.channel_id);
            if (game && lfgChannelTable) {
                const interfaceMessage = await fetchMessage(message.channel, interfaceTable.message_id);
                if (interfaceMessage) {
                    try {
                        await send_lfg_interface(message.channel as TextChannel, game.game_name, lfgChannelTable);
                        await handle_delete_interface(interfaceMessage); // delete the old interface
                    } catch (error) {
                        await errorLogHandle(error);
                    }
                }
            } else {
                console.error("Failed to fetch the game or channel using the interface while bumping")
            }
        } else {
            console.error("Failed to fetch the interface table while bumping");
        }
        return cooldown;
    } else {
    ...
    const collector = await message_collector<ComponentType.Button>(message,
        {
            ...
            time: duration_to_seconds("2h")! * 1000,
...
export async function lfg_post_builder(
    ...
    try {
        ...
        // TODO: ADD PROPER DATA HANDLING
        // delete and re-send the interface
        const interfaceTable = await LfgSystemRepo.getInterfaceByChannelId(lfgChannel.id);
        if (interfaceTable) {
            const game = await LfgSystemRepo.getGameByInterface(interfaceTable.message_id);
            const lfgChannelTable = await LfgSystemRepo.getLfgChannelById(interfaceTable.channel_id);
            if (game && lfgChannelTable) {
                const interfaceMessage = await fetchMessage(channel, interfaceTable.message_id);
                if (interfaceMessage) {
                    try {
                        await send_lfg_interface(channel as TextChannel, game.game_name, lfgChannelTable);
                        await handle_delete_interface(interfaceMessage); // delete the old interface
                    } catch (error) {
                        await errorLogHandle(error);
                    }
