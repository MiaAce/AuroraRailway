        ...
        if (interaction.isChatInputCommand() && interaction.member instanceof GuildMember && interaction.guild) {
            ...
            if (command.metadata.ownerOnly === true && interaction.user.id !== get_env_var("OWNER")) {
                return await interaction.reply({
                    embeds: [embed_error("This command requires BOT Owner privileges!")],
                    flags: MessageFlags.Ephemeral
                });
            }

            const guild: Guild = botMember.guild;

            if (command.metadata.group === "premium") { // all premium commands are under the premium group
                const premiumRoleId = await ServerRolesRepo.getGuildPremiumRole(guild.id);
                if (
                    premiumRoleId &&
                    command.metadata.category !== "Administrator" && // commands under admin category should not require premium membership
                ...
                // block premium commands while there is no premium role set up
            ...
            if (command.metadata.cooldown > 0) set_cooldown(interaction.user.id, timestamps, command.metadata.cooldown);
