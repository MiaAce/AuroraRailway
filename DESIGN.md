...
class LfgSystemRepository {
    ...
    async setGameSnowflakes(game: LfgGameTable): Promise<LfgGameTable> {
        const { rows: response } = await database.query<LfgGameTable>(
            `UPDATE lfg_games SET category_channel_id=$2 WHERE id=$1
            RETURNING *;`,
            [game.id, game.category_channel_id]
        );
     ...
     */
    async getGameByInterface(messageId: Snowflake): Promise<LfgGameTable | null> {
        const { rows: data } = await database.query<LfgGameTable>(
            `
            SELECT g.*
            FROM lfg_interfaces i
            JOIN lfg_channels c ON c.id = i.channel_id
            JOIN lfg_games g ON g.id = c.game_id
    ...
    async getLfgChannelById(id: number): Promise<LfgChannelTable | null> {
        const { rows: data } = await database.query<LfgChannelTable>(
            `SELECT * FROM lfg_channels WHERE id=$1`, [id]
        );
        return data[0]!;
    }
    ...
    async getLfgChannelBySnowflake(id: Snowflake): Promise<LfgChannelTable | null> {
        const { rows: data } = await database.query<LfgChannelTable>(
            `SELECT * FROM lfg_channels WHERE discord_channel_id=$1`, [id]
        );
        return data[0]!;
    }
    ...
    // =========================
    // lfg_interfaces related repos
    // =========================

    async registerInterface(messageId: string, lfgChannelId: number): Promise<LfgInterface> {
        const { rows: data } = await database.query<LfgInterface>(
            `INSERT INTO lfg_interfaces(message_id, channel_id)
            VALUES($1, $2)
            RETURNING *;`,
            [messageId, lfgChannelId]
        );

        return data[0]!;
    }

    async deleteInterface(messageId: string) {
        await database.query(`DELETE FROM lfg_interfaces WHERE message_id=$1`, [messageId]);
    }

    async fetchInterfacesWithChannels(): Promise<LfgInterfaceWithChannel[]> {
        const { rows: data } = await database.query<LfgInterfaceWithChannel>(
            `SELECT 
                i.message_id,
    ...
    async getInterfaceByChannelId(id: number): Promise<LfgInterface | null> {
        const { rows: data } = await database.query<LfgInterface>(
            `SELECT * FROM lfg_interfaces WHERE channel_id=$1`, [id]
        );
