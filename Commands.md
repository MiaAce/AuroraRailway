...
import {
    ...
    resolveSnowflakesToChannels,
    resolveSnowflakesToRoles
} from "../../utility_modules/discord_helpers.js";
import { send_lfg_interface } from "../../Systems/lfg/lfg_interface_manager.js";
    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommandGroup(subcommandGroup =>
            subcommandGroup.setName("assign")
                ...
                .addSubcommand(subcommand =>
                    subcommand.setName("lfg-interfaces")
                        .setDescription("Send interfaces in the selected channels")
                )
        )
        .addSubcommandGroup(subcommandGroup =>
    ...
    async execute(interaction) {
        ...
        switch (subcommand) {
            ...
            case "builder": {
                ...
                const unBuiltGames = guildGames.filter((row) => row.category_channel_id === null);
                ...
                switch (subcommand) {
                    ...
                    case "channel": {
                        ...
                        try {
                            ...
                            const registeredChannel = await LfgSystemRepo.registerChannel(newLfgChannel);

                            // send interface
                            await send_lfg_interface(newChannel, selectedGame.game_name, registeredChannel);
                    ...
                    case "lfg-interfaces": {
                        ...
                        const collector = await message_collector<ComponentType.StringSelect>(reply,
                            ...
                            async (selectInteraction) => {
                                ...
                                const gameSelected = builtGames.find(g => g.id === gameId)!;
                                const channels = await LfgSystemRepo.getLfgChannelsByGame(gameId);
                                // TODO VALIDATE THE CHANNELS BEFORE SENDING THE INTERFACE
                                // MAKE SURE THERE ISN'T AN INTERFACE ALREADY THERE
                                const modal = new ModalBuilder()
                                    .setCustomId("modal-interfaces")
                                    .setTitle("Send LFG interfaces")
                                ...
                                try {
                                    ...
                                    submit.deferReply({ flags: MessageFlags.Ephemeral });
                                    const channelIds = submit
                                        .fields
                                        .getStringSelectValues("select-channel-menu")
                                        .map(id => Number(id));

                                    const channelSnowflakes = channels
                                        .filter(c => channelIds.includes(c.id))
                                        .map(c => c.discord_channel_id!);

                                    const lfgChannelsResolved = await resolveSnowflakesToChannels<TextChannel>(
                                        guild,
                                        channelSnowflakes,
                                        isTextChannel
                                    );

                                    for (const ch of lfgChannelsResolved) {
                                        const channelTable = channels.find(c => c.discord_channel_id === ch.id)!;
                                        await send_lfg_interface(
                                            ch,
                                    ...
                                    await submit.editReply({
                                        embeds: [embed_message("Green", "Interfaces sent.")]
                                    });
                                    collector.stop();
                                } catch (error) {
                                    console.error(error); // remove after dev
                        ...
                        const collector = await message_collector<ComponentType.StringSelect>(reply,
                            ...
                            async (selectInteraction) => {
                                ...
                                const selectedBuiltGames = guildGames.filter(
                                    g => ids.includes(g.id) && g.category_channel_id !== null
                                );
                                for (const row of selectedBuiltGames) { // attempt to delete the interface channel of built games
                                    const interfaceChannel = await fetchGuildChannel(guild, row.category_channel_id!);
                                    if (interfaceChannel) {
                                        try {
